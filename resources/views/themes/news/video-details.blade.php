                    ...
                    @foreach ($recentPostsChunkedNews as $index => $recentPost)
                        ...
                        @if ($recentPost->isNotEmpty())
                            ...
                            {{-- <div class="{{ $paddingClass }}">
                                ...
                                {{-- path thik thak ache but image asche na. 
                                http://localhost/trust/assets/thumbnail-image.jpg dea check kore dekha. --}}
                             
                                {{-- <a href="{{ __url(optional($recentPost->first())->encode_title) }}"
   class="block w-full h-64 group overflow-hidden">
    <img class="w-full h-full object-cover group-hover:scale-105 transition_5"
         src="
            @if(!empty($recentPost->first()) && !empty($recentPost->first()->thumbnail_image))
                {{ asset('storage/'.$recentPost->first()->thumbnail_image) }}
            @else
                {{ asset('assets/thumbnail-image.jpg') }}
            @endif
         "
         alt="{{ optional($recentPost->first())->image_alt ?? 'Thumbnail Image' }}" />
</a> --}}
